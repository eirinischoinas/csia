...
TUYAAPISECRET="xxxxxxxxxxxxxxxxxxxxx"
TUYAAPIDEVICEID="27225844840d8e7aabae"

# main variables
# how often grid is checked
polltime = 5
...
def grid_down(powerwall):
	...
	if (format(powerwall.get_grid_status()) == 'GridStatus.CONNECTED'):
		return False
	else:
		return True
...
if __name__ == '__main__':
	...
	monitorWindow.addstr(format(powerwall.get_grid_status()) + '\n')
	monitorWindow.refresh()

	# present powerwall information
	commandWindow.addstr('Powerwall: ' + PWIPNAME + '\n')
	commandWindow.addstr('powerwall capacity (watts): ' + format(powerwall.get_capacity()) + '\n')
